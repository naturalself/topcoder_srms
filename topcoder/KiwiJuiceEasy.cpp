...
class KiwiJuiceEasy {
public:
	vector <int> thePouring(vector <int> cap, vector <int> bot, vector <int> fromId, vector <int> toId) {

	vector <int> ret((int)cap.size());

	forv(i,cap){
		ret[i]=min(cap[i],bot[i]);		
	}
	
	forv(i,fromId){
		int move = min(ret[fromId[i]],(cap[toId[i]]-ret[toId[i]]));
		ret[fromId[i]] -= move;
		ret[toId[i]] += move;
	}
