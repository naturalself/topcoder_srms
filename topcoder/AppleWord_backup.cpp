		...
		if(word.size() < 5){
			...
			if(apple[0]==word[0] || (apple[0]+32)==word[0]){	//a or A?
				for(int i=1;i < word.size();i++){
					...
					if(apple[1] != word[i] || (apple[1]+32) != word[i]){	//p or P?
						
						//For apple word
						if(word.substr(i,1)=="l" || word.substr(i,1)=="L"){	//l or L?
							
							cout << "L" << endl;
							
							if(word.substr(i+1,1)=="e" || word.substr(i+1,1)=="E"){	//e or E?
