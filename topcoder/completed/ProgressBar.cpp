...
public:
	...
	int sum = accumulate(taskTimes.begin(),taskTimes.end(),0);
	int cur = 0;
	int bar_num = 0;

	for(int i=0;i<tasksCompleted;i++) cur += taskTimes[i];
	bar_num =(int)cur*20/sum;

	return string(bar_num,'#') + string(20-bar_num,'.');
	}
