...
public:
	...
	for(int i=0;i<(int)scoreSheet.size();i++){	
		if(scoreSheet[i] == "triple bogey"){
			strokes += parValues[i]+3;
		}else 
		if(scoreSheet[i] == "double bogey"){
			strokes += parValues[i]+2;
		}else 
		if(scoreSheet[i] == "bogey"){
			strokes += parValues[i]+1;
		}else 
		if(scoreSheet[i] == "par"){
			strokes += parValues[i];
		}else 
		if(scoreSheet[i] == "birdie"){
			strokes += parValues[i]-1;
		}else 
		if(scoreSheet[i] == "eagle"){
			strokes += parValues[i]-2;
		}else 
		if(scoreSheet[i] == "albatross"){
			strokes += parValues[i]-3;
		}else 
