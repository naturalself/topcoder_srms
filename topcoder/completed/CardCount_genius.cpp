...
public:
	...
	int n=(int)(deck).size()/numPlayers*numPlayers;

	//int n=(int)(deck).size()/numPlayers;

	cout << "(int)(deck).size() =" << (int)(deck).size() << endl;

	cout << "numPlayers =" << numPlayers << endl;

	cout << "(int)(deck).size()/numPlayers =" << (int)(deck).size()/numPlayers << endl;

	cout << "n=" << n << endl;

	for(int i=0;i<n;i++) res[i%numPlayers] += deck[i];
