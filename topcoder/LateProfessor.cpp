...
class LateProfessor {
public:
	double getProbability(int wait, int walk, int late, int bestA, int worstA) {
		
	if(bestA == worstA){
		int t = bestA%(wait+walk);
		if(t > wait && walk+wait-t >= late){
			return 1;
		}else{
			return 0;
		}
